...
- **점검 시각을 스냅샷에 넣지 않는다.** 스냅샷은 내용이 바뀐 날만 바뀌고, 그 날은
  어차피 Claude가 도는 날이다. (매일 움직이는 값은 curated.json의 dataTimestamp
  하나뿐이며, 그건 워크플로우가 건너뛴 날에도 따로 갱신한다 — 앱의 "기준 날짜"
  라벨용. 이 스크립트는 curated.json을 쓰지 않는다.)
